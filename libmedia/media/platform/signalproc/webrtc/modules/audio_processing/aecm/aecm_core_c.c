...
static int TimeToFrequencyDomain(AecmCore_t* aecm,
                                 const int16_t* time_signal, // 64 uint16_t elements of time signal part
                                 complex16_t* freq_signal,   // 64 complex16_t elements array
                                 uint16_t* freq_signal_abs,  // 65 uint16_t elements
                                 uint32_t* freq_signal_sum_abs)
{
    ...
    // return how many leading zero of tmp16no1
    // only [0, 15] is calacuted
    ...
    freq_signal_abs[PART_LEN] = (uint16_t)WEBRTC_SPL_ABS_W16(freq_signal[PART_LEN].real);
